        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.epochs):
                ...
                for i in range(1, 6):
                    ...
                    batch_data = cifar.load_preprocess_data(self.prepro_train_path, data_name)
                    for feature, label in batch_data:
                        sess.run(self.optimizer, feed_dict={self.x: feature,
                                                            self.y: label,
                                                            self.keep_prob: cfg.hyperparam['keep_prob']})
                        index = index + 1
                        if index % 1000 == 0:
                            loss = sess.run(loss, feed_dict={self.x: feature,
                                                             self.y: label,
                                                             self.keep_prob: 1})
